...
def pay():
    ...
    if int(amount)>=int(rate):
        ...
        pnr_list=[]
        sql="select * from ticket where username=%s"
        cursorObject.execute(sql,[(username.get())])
        table=cursorObject.fetchall()
        ...
        sql = "INSERT INTO ticket (username, pnr, source, destination, rate) VALUES (%s, %s, %s, %s, %s)"
        val = (username.get(), pnr, clicked.get(), clicked1.get(), s)
