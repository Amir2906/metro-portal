...
def check_activity():
    ...
    if submit==1: #to enable disable otp button
...
def on_submit():
    ...
    if int(otp)==int(send_otp.get()):
        ...
        sql="insert into login (`first_name`, `last_name`, `username`, `password`, `confirm_password`, `age`, `contact`) values(%s, %s, %s, %s, %s, %s, %s)"
        val = (f_name.get(), l_name.get(), username.get(), password.get(), confirm_password.get(), age.get(), contact.get())
        print(val)
...
def send_otp():
    ...
    account_sid = 'AC9e92bcbb56fc53295ff3be54d671e43c'
    auth_token = 'cb13dd721dea47b5f09df8e9fffa51c7'
